#!/usr/bin/env node
/**
 * Claude Team - MCP Bridge
 *
 * MCP server exposing Claude Team operations as tools.
 * Two categories:
 *   1. State readers  - read pipeline state JSON (no compilation needed)
 *   2. Logic tools    - call compiled TypeScript modules via dynamic import()
 *
 * Tools (23):
 *   State readers:  ct_team_status, ct_kanban_board, ct_quality_summary, ct_role_info, ct_sprint_status, ct_get_plan_status
 *   Logic tools:    ct_analyze_complexity, ct_select_roles, ct_build_dag, ct_validate_transition,
 *                   ct_score_review, ct_aggregate_reviews, ct_gate_verdict,
 *                   ct_get_agent_prompt, ct_decompose_task, ct_escalation_decision, ct_list_roles
 *   State writers:  ct_init_pipeline, ct_update_state, ct_transition_phase
 *   Orchestration:  ct_build_team, ct_register_team, ct_run_pwj_cycle
 */
...
/**
 * Validate and sanitize a cwd path to prevent directory traversal.
 * Returns resolved absolute path or throws on invalid input.
 *
 * Security layers:
 * 1. Type + null byte check
 * 2. Path traversal rejection (post-resolution)
 * 3. Sensitive system path blocklist
 * 4. cwd-subtree restriction (must be under process.cwd())
 * 5. Maximum path length enforcement
 */
function validateCwd(cwd) {
  ...
  // Max path length (prevent DoS via extremely long paths)
  if (cwd.length > 4096) {
    throw new Error('cwd path exceeds maximum length (4096)');
  }
  const resolved = resolve(cwd);
  // Reject paths that try to escape via .. after resolution
  // The path must be an absolute path to a real project directory
  if (resolved.includes('..')) {
    throw new Error('cwd must not contain path traversal sequences');
  }
  // cwd-subtree restriction: resolved path must be under process.cwd()
  ...
  // Reject common sensitive system paths (allow /var/folders for macOS temp)
...
/**
 * Validate string arguments (non-empty, reasonable length).
 */
function validateString(value, name, maxLen = 10000) {
...
/**
 * Validate number arguments (finite, within range).
 */
...
// Lazy-loaded ESM modules (populated on first logic tool call)
let _ct = null;
...
// ============================================================
// STATE READER HANDLERS
// ============================================================

/**
 * Read pipeline state via compiled module.
 * Delegates to ct.loadPipelineState which handles schema validation.
 * Falls back to direct JSON read if compiled modules are unavailable.
 */
async function readState(cwd) {
  ...
  try {
    ...
    // Fallback: direct JSON read if compiled modules not available
  ...
  // Compiled modules available — delegate directly (surfaces corrupt state errors)
...
// ============================================================
// STATE WRITER HANDLERS
// ============================================================
// All state mutations delegate to compiled TypeScript modules which provide:
// - Advisory file locking (withStateLock)
// - Atomic writes (write-to-temp-then-rename)
// - Prototype pollution protection (safeAssign with FORBIDDEN_KEYS)
// - Schema version consistency (TEAM_PIPELINE_SCHEMA_VERSION)
...
async function handleInitPipeline(args) {
  ...
  // Delegate to compiled module — inherits file locking, atomic writes, schema version
...
async function handleUpdateState(args) {
  ...
  // Single lock for the entire update to prevent race conditions
  // between concurrent handleUpdateState calls
  return ct.withStateLock(safeCwd, () => {
    ...
    if (Array.isArray(args.roles)) {
      ...
      const validRoles = args.roles
        .filter(r => r && typeof r === 'object' && typeof r.role === 'string')
        .map(r => {
          const clean = {};
...
async function handleTransitionPhase(args) {
  ...
  // Delegate to compiled module — inherits file locking, transition validation,
  // fix loop management, terminal state handling, atomic writes
...
async function handleAnalyzeComplexity(args) {
  ...
  if (args.factors) {
    const result = ct.analyzeComplexity({
...
async function handleBuildDag(args) {
  ...
  // plan.layers is already topologically sorted by buildExecutionPlan
...
async function handleValidateTransition(args) {
  ...
  validateString(args.reason, 'reason', 2000);
  ...
  const fullValidation = ct.validateTransition({
    ...
    reason: args.reason,
    gateVerdict: args.gateVerdict || undefined,
  });
...
async function handleScoreReview(args) {
  validateString(args.response, 'response', 100000);
...
async function handleGateVerdict(args) {
  ...
  if (args.score === undefined || args.score === null) return err('score is required');
  if (!args.dimensions || typeof args.dimensions !== 'object') return err('dimensions object is required');
  if (args.attempt !== undefined) validateNumber(args.attempt, 'attempt', 1, 10);
...
async function handleGetAgentPrompt(args) {
  ...
  // Without preamble: load only the role-specific prompt
...
async function handleEscalationDecision(args) {
  const safeCwd = validateCwd(args.cwd);
  validateString(args.taskId, 'taskId', 200);
  validateString(args.feedback, 'feedback', 100000);
  validateNumber(args.score, 'score', 0, 10);

  const ct = await loadModules();
  // Initialize SQLite so determineEscalation can query gate history
...
async function handleRegisterTeam(args) {
  validateString(args.teamName, 'teamName', 200);
  validateString(args.description, 'description', 1000);
  ...
  // Derive layers from member dagLayer if not explicitly provided
  ...
  // Reconstruct TeamConfiguration for registerTeam
  const teamConfig = {
    members: members,
    mergeLog: [],
...
async function handleRunPwjCycle(args) {
  ...
  // Select roles based on complexity (same logic as selectRoles in planner-worker-judge.ts)
  const roles = ct.selectRoles(args.complexity);

  // Build a task description summary for prompts
  const taskDescription = args.taskSpecs.map((t, i) =>
    `Task ${i + 1}: ${t.title || t.id}${t.description ? ' - ' + t.description : ''}`
  ).join('\n');

  // Rework prefix for prompts when this is a re-cycle
  const reworkPrefix = reworkCount > 0 && previousFeedback
    ? `This is rework cycle #${reworkCount}. Previous judgment feedback:\n${previousFeedback}\n\nAddress all issues raised before proceeding.\n\n`
    : '';

  // Helper: get agent prompt text from ct module
  ...
  // Derive role groups from assignments
  ...
  // Build merged role label (e.g. "pm, pl" for tiny where all merge)
  function roleLabel(roleAssignments) {
    return roleAssignments.map(r => r.role).join(', ');
  }

  // Planning phase prompt — use first planner agent's base prompt
...
Provide output as structured JSON or markdown that worker agents can directly act on.`;
  ...
  // Execution phase prompt — use first worker agent's base prompt
...
Provide implementation artifacts and a summary of what was done.`;
  ...
  // Judgment phase prompt — use first judge agent's base prompt
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
      switch (name) {
        ...
        case 'ct_team_status':       return await handleTeamStatus(args);
        case 'ct_kanban_board':      return await handleKanbanBoard(args);
        case 'ct_quality_summary':   return await handleQualitySummary(args);
        case 'ct_role_info':         return await handleRoleInfo(args);
        case 'ct_sprint_status':     return await handleSprintStatus(args);
        // Logic tools
        case 'ct_analyze_complexity':  return await handleAnalyzeComplexity(args);
        case 'ct_select_roles':        return await handleSelectRoles(args);
        case 'ct_build_dag':           return await handleBuildDag(args);
        case 'ct_validate_transition': return await handleValidateTransition(args);
        case 'ct_score_review':        return await handleScoreReview(args);
        case 'ct_aggregate_reviews':   return await handleAggregateReviews(args);
        case 'ct_gate_verdict':        return await handleGateVerdict(args);
        case 'ct_get_agent_prompt':    return await handleGetAgentPrompt(args);
        case 'ct_decompose_task':      return await handleDecomposeTask(args);
        case 'ct_escalation_decision': return await handleEscalationDecision(args);
        case 'ct_list_roles':          return await handleListRoles();
        // State writers (async — delegate to compiled modules with file locking)
        case 'ct_init_pipeline':       return await handleInitPipeline(args);
        case 'ct_update_state':        return await handleUpdateState(args);
        case 'ct_transition_phase':    return await handleTransitionPhase(args);
        // Orchestration tools
        case 'ct_build_team':          return await handleBuildTeam(args);
        case 'ct_register_team':       return await handleRegisterTeam(args);
        case 'ct_get_plan_status':     return await handleGetPlanStatus(args);
        case 'ct_run_pwj_cycle':       return await handleRunPwjCycle(args);
        default:
